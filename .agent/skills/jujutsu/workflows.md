...
# 2. Create bookmark and push in one command (--named recommended)
jj git push --named feature/my-feature=@
...
Note: The `--named` flag creates the bookmark, tracks it, and pushes it in one step. This is the recommended approach for first-time pushes.
...
# Show evolution history of a commit (formerly jj obslog)
jj evolog  # or jj evolution-log
```

Note: `jj obslog` is deprecated as of v0.17. Use `jj evolog` or `jj evolution-log` instead.
