...
**TODO:**
- [x] カバレッジの測定 (`go test -cover ./...`)
- [x] 十分なカバレッジが確保されていることを確認

### 6.3 API 動作確認（手動テスト）

**TODO:**
- [x] サーバーの起動 (`go run cmd/main.go`)
- [x] `POST /todo`: Todo の作成が成功することを確認
- [x] `GET /todo/list`: 作成した Todo が一覧に含まれていることを確認
- [x] `GET /todo/{id}`: 特定の Todo が ID 指定で取得できることを確認
- [x] `PUT /todo/{id}`: Todo の内容（タイトルや完了状態）を更新できることを確認
- [x] `DELETE /todo/{id}`: Todo を削除できることを確認
